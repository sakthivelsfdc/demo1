...
export default class CovidTrackerMain extends LightningElement {
    ...
    getCovidSummaryData(){ 
        getCovidSummary()
            .then(result => {
                    ...
                    if(this.jsonObj.errors) {
                        console.log('errors:' + JSON.stringify(this.jsonObj.errors));
                        this.errors = this.jsonObj.errors;
                        this.jsonObj = undefined;
                        this.showError = true;
                        this.showSpinner = false;

                        this.handleShowToastEvent('Covid19 Summary','Error- please refer the error log', 'error');
                    } else {
    ...
    handleShowToastEvent(title, message, variant, messageData) {
        this.dispatchEvent(
            new ShowToastEvent({
                title: title,
                message: message,
